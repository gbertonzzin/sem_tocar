...
import qrcode                           
from pyzbar.pyzbar import decode        #used for decoding, maybe use qrcode for this as well?
...
def encrypt_data (cal_id, eve_id):
    ...
    jumbled = ("".join(i for j in zip_longest(cal_id, eve_id, fillvalue=filler) for i in j))[::-1]
...
def decrypt_data(input):
    ...
    unjumbled_cal_id = (decrypted[1::2])[::-1]#.replace("$","")#for use with CRYPTO_FILLER
